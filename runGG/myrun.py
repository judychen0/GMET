...
with open(remotepath, "r") as filepath:
    ...
    for line in path:
        ...
        script += "\n"
        script += """/usr/bin/time parallel --progress --jobs 1 'root -L -l -b -q "xZg1.C+(\\\"{1}\\\", {2})"' :::: path.txt ::: $isSig :::+ 1 \n"""
        script += """/usr/bin/time parallel --progress --jobs 10 'root -L -l -b -q "xZg1.C+(\\\"{1}\\\", {2})"' :::: path.txt ::: $isSig \n"""
        ...
        frun = open("run"+str(yeardir)+"_Zg_"+str(isDcount)+"_"+str(isData)+"_"+str(isSignal)+".sh", "w")
        frun.write(script)
        frun.close()
        
        os.system( "chmod 744 run"+str(yeardir)+"_Zg_"+str(isDcount)+"_"+str(isData)+"_"+str(isSignal)+".sh" )
        os.system( "./run"+str(yeardir)+"_Zg_"+str(isDcount)+"_"+str(isData)+"_"+str(isSignal)+".sh")
