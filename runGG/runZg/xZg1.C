	...
	if(pho_sel(cutflow[icat],16) == 1) {
	  ...
	  if(icat == 1){
	    for(Int_t jj=0; jj<npfjet; jj++){
	      Int_t ijet = nonPUjetid[jj];
	      if(jj>1) continue;
	      h_djetMETPhi_cut[jj]->Fill(JMETdPhi[ijet], WEIGHT);
	      h_dr_phojet[jj]->Fill(phojetdR[ijet], WEIGHT);
	      h_dEta_phojet[jj]->Fill(phojetdEta[ijet], WEIGHT);
	      h_dPhi_phojet[jj]->Fill(phojetdPhi[ijet], WEIGHT);
	      if((phoEt[ipho]/corrMET) > 2) h_jetpt_ptoverMET_CR->Fill(jetPt[ijet], WEIGHT);
	    }
	    ...
	    //2d plots
	    h2_MET_minJMETdPhi->Fill(corrMET, minJMETdPhi, WEIGHT);
	    h2_MET_dijetMass->Fill(corrMET, dijetMass, WEIGHT);
	    h2_MET_ptoverMET->Fill(corrMET, phoEt[ipho]/corrMET, WEIGHT);
	    h2_dijetMass_minJMETdPhi->Fill(dijetMass, minJMETdPhi, WEIGHT);
	    h2_dijetMass_ptoverMET->Fill(dijetMass, phoEt[ipho]/corrMET, WEIGHT);
	...
	//VBS Nm1 plots
	  ...
	  if(Nm1_sel(cutflow[icat],15) == 1){
	    for(Int_t jj=0; jj<npfjet; jj++){
	      if(jj>1) continue;
	      Int_t ijet = nonPUjetid[jj];
	      h_djetMETPhi_Nm1[jj]->Fill(JMETdPhi[ijet], WEIGHT);
	    }
	  ...
	  if(Nm2_sel(cutflow[icat], 7, 15) == 1) h_MET_Nm1_minJMETdPhi_SB->Fill(corrMET, WEIGHT);
	}
