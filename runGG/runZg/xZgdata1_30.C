...
void xZgdata1(char* pathes){
  ...
  //check
  Int_t ptoverMET_count = 0;
  Int_t ptoverMET_CR_count = 0;
  ...
  for(Int_t j=0; j<2; j++){
    ...
    h_ptoverMET_cut[j] = new TH1F(Form("h_ptoverMET_cut_%i", j), "phoEB pt/MET N cut", 60, 0, 12);
    h_ptoverMET_Nm1[j] = new TH1F(Form("h_ptoverMET_Nm1_%i", j), "phoEB pt/MET N-1 cut", 60, 0, 12);
    h_ptoverMET_cut[j]->Sumw2();
    h_ptoverMET_Nm1[j]->Sumw2();
  ...
  h_dr_phojet = new TH1F("h_dr_phojet", "pho jet dr", 20, 0., 8);
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	fillpho[icat] = 0;
	if(pho_sel(cutflow[icat], 16) == 1) fillpho[icat] += 1;
	...
	if(pho_sel(cutflow[icat],14) == 1) h_phoEB_pt_phoCentral[icat]->Fill(phoEt[ipho], WEIGHT);
	...
	if(pho_sel(cutflow[icat],16) == 1) {
	  ...
	  h_MET_cut[icat]->Fill(puppiMET, WEIGHT);
	  h_METPhi_cut[icat]->Fill(puppiMETPhi, WEIGHT);
	  h_dphoMETPhi_cut[icat]->Fill(phoMETdPhi[ipho], WEIGHT);
	  h_ptoverMET_cut[icat]->Fill(phoEt[ipho]/puppiMET, WEIGHT);
	  ...
	  if(icat == 1){
	    for(Int_t jj=0; jj<npfjet; jj++){
	      ...
	      if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 12 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 20) h_jetpt_ptoverMET_CR->Fill(jetPt[ijet], WEIGHT);
	      else if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11) h_jetpt_ptoverMET_SR->Fill(jetPt[ijet], WEIGHT);
	      //if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11) h_jetpt_ptoverMET_SR->Fill(jetPt[ijet], WEIGHT);
	    }
	    ...
	    //if((phoEt[ipho]/puppiMET) >= 2 && (phoEt[ipho]/puppiMET) <= 4){//ptoverMET CR
	    if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 12 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 20){//ptoverMET CR
	      ptoverMET_CR_count++;
	      h_phoEB_ptcut_ptoverMET_CR->Fill(phoEt[ipho], WEIGHT);
	      h_MET_ptoverMET_CR->Fill(puppiMET, WEIGHT);
	    ...
	    //else if((phoEt[ipho]/puppiMET) < 2){//ptoverMET SR
	    else if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11){//ptoverMET SR
	      //if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11){//ptoverMET SR
	      h_phoEB_ptcut_ptoverMET_SR->Fill(phoEt[ipho], WEIGHT);
	      h_MET_ptoverMET_SR->Fill(puppiMET, WEIGHT);
	...
	if(Nm1_sel(cutflow[icat],7) == 1){
	  h_MET_Nm1[icat]->Fill(puppiMET, WEIGHT);
	  h_METPhi_Nm1[icat]->Fill(puppiMETPhi, WEIGHT);
	}
	...
	if(Nm1_sel(cutflow[icat],14) == 1) h_ptoverMET_Nm1[icat]->Fill(phoEt[ipho]/puppiMET, WEIGHT);
	...
	//VBS Nm1 plots
	  ...
	  if(Nm2_sel(cutflow[icat],10,11) == 1) h_dEta_jetjet_Nm1->Fill(jetjetdEta, WEIGHT);
	  ...
	  if(Nm1_sel(cutflow[icat],14) == 1) h_phoCentral_Nm1->Fill(phoCentral, WEIGHT);

	  if(Nm2_sel(cutflow[icat], 7, 15) == 1 && minJMETdPhi<0.5) h_MET_Nm1_minJMETdPhi_SB->Fill(puppiMET, WEIGHT);
	  if(Nm1_sel(cutflow[icat], 7) == 1) h_MET_Nm1_minJMETdPhi_SR->Fill(puppiMET, WEIGHT);

	  //2d plots
	  if(Nm2_sel(cutflow[icat],7,15) == 1) h2_MET_minJMETdPhi->Fill(puppiMET, minJMETdPhi, WEIGHT);
	  if(Nm2_sel(cutflow[icat],7,16) == 1) h2_MET_dijetMass->Fill(puppiMET, dijetMass, WEIGHT);
	  if(Nm2_sel(cutflow[icat],7,14) == 1) h2_MET_ptoverMET->Fill(puppiMET, phoEt[ipho]/puppiMET, WEIGHT);
	  if(Nm2_sel(cutflow[icat],15,16) == 1) h2_dijetMass_minJMETdPhi->Fill(dijetMass, minJMETdPhi, WEIGHT);
	  if(Nm2_sel(cutflow[icat],14,16) == 1) h2_dijetMass_ptoverMET->Fill(dijetMass, phoEt[ipho]/puppiMET, WEIGHT);
	  if(Nm2_sel(cutflow[icat],14,15) == 1) h2_minJMETdPhi_ptoverMET->Fill(minJMETdPhi, phoEt[ipho]/puppiMET, WEIGHT);

	  if(Nm1_sel(cutflow[icat], 15) == 1 && minJMETdPhi<0.5){
	    ...
	    if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 12 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 20){//ptoverMET CR
	      ptoverMET_CR_count++;
	      h_phoEB_ptcut_ptoverMET_CR_minJMETdPhi_SB->Fill(phoEt[ipho], WEIGHT);
	      h_MET_ptoverMET_CR_minJMETdPhi_SB->Fill(puppiMET, WEIGHT);
	      h_dphoMETPhi_ptoverMET_CR_minJMETdPhi_SB->Fill(phoMETdPhi[ipho], WEIGHT);
	      h_dijetMass_ptoverMET_CR_minJMETdPhi_SB->Fill(dijetMass, WEIGHT);
	      h_dEta_jetjet_ptoverMET_CR_minJMETdPhi_SB->Fill(jetjetdEta, WEIGHT);
	      h_minJMETdPhi_ptoverMET_CR_minJMETdPhi_SB->Fill(minJMETdPhi, WEIGHT);
	    }
	    
	    if(h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut[icat]->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11){//ptoverMET SR
	      h_phoEB_ptcut_ptoverMET_SR_minJMETdPhi_SB->Fill(phoEt[ipho], WEIGHT);
	      h_MET_ptoverMET_SR_minJMETdPhi_SB->Fill(puppiMET, WEIGHT);
	      h_dphoMETPhi_ptoverMET_SR_minJMETdPhi_SB->Fill(phoMETdPhi[ipho], WEIGHT);
	      h_dijetMass_ptoverMET_SR_minJMETdPhi_SB->Fill(dijetMass, WEIGHT);
	      h_dEta_jetjet_ptoverMET_SR_minJMETdPhi_SB->Fill(jetjetdEta, WEIGHT);
	      h_minJMETdPhi_ptoverMET_SR_minJMETdPhi_SB->Fill(minJMETdPhi, WEIGHT);
	    }
	  }
  ...
  fout_->mkdir("ptoverMET_CR_minJMETdPhi_SB");
  fout_->cd("ptoverMET_CR_minJMETdPhi_SB");

  h_phoEB_ptcut_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_jetpt_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_MET_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_dphoMETPhi_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_dijetMass_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_dEta_jetjet_ptoverMET_CR_minJMETdPhi_SB->Write();
  h_minJMETdPhi_ptoverMET_CR_minJMETdPhi_SB->Write();
  ...
  fout_->mkdir("ptoverMET_SR_minJMETdPhi_SB");
  fout_->cd("ptoverMET_SR_minJMETdPhi_SB");

  h_phoEB_ptcut_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_jetpt_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_MET_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_dphoMETPhi_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_dijetMass_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_dEta_jetjet_ptoverMET_SR_minJMETdPhi_SB->Write();
  h_minJMETdPhi_ptoverMET_SR_minJMETdPhi_SB->Write();
  ...
  fout_->Close();
  //cout << "ptoverMET_CR events : " << ptoverMET_CR_count << endl;
