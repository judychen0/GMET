...
void xSkim_data(char* pathes, Int_t year){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    if(npfjet>=3){
      ...
      leadjet.SetPtEtaPhi(jetPt_[ilead], jetEta_[ilead], jetPhi_[ilead]);
      subjet.SetPtEtaPhi(jetPt_[isub], jetEta_[isub], jetPhi_[isub]);

      jetPt_corr.push_back(jetPt_[ilead]);
      jetPt_corr.push_back(jetPt_[isub]);
	...
	//if(jj>=2){
	  ...
	  smalljet.SetPtEtaPhi(jetPt_[ijet], jetEta_[ijet], jetPhi_[ijet]);
	  ...
	  if(count == 0){
	    jetPt_corr.push_back(jetPt_[ijet]);
    ...
    if(npfjet_corr >= 2){
      ...
      //mindjetMETPhi
      if(fabs(deltaPhi(jetPhi_corr[ilead], puppiMETPhi)) < fabs(deltaPhi(jetPhi_corr[isub], puppiMETPhi))) minJMETdPhi = deltaPhi(jetPhi_corr[ilead], puppiMETPhi);
      else if(fabs(deltaPhi(jetPhi_corr[ilead], puppiMETPhi)) > fabs(deltaPhi(jetPhi_corr[isub], puppiMETPhi))) minJMETdPhi = deltaPhi(jetPhi_corr[isub], puppiMETPhi);
      ...
      //dijet Mass
      leadjetP4.SetPtEtaPhiE(jetPt_corr[ilead], jetEta_corr[ilead], jetPhi_corr[ilead], jetEn_[ilead]);
      subjetP4.SetPtEtaPhiE(jetPt_corr[isub], jetEta_corr[isub], jetPhi_corr[isub], jetEn_[isub]);
	...
	phojetdPhi.push_back(deltaPhi(phoPhi[ipho], jetPhi_corr[isub]));
      ...
      //jetMET dPhi
      JMETdPhi.push_back(deltaPhi(jetPhi_corr[ilead], puppiMETPhi));
      JMETdPhi.push_back(deltaPhi(jetPhi_corr[isub], puppiMETPhi));
    }
	...
	if(jetPt_corr[nonPUjetid[0]] >30) {bit = SetBit(9, bit); nSMjet = npfjet_corr;}
	else if(jetPt_corr[nonPUjetid[0]] <30) {bit = SetBit(9, bit); nSMjet = 0;}
      }
