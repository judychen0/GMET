...
with open(remotepath, "r") as pathfile:
    ...
    for line in path:
        ...
        script += """for filename in $(cat path.txt); do outfile=output_$filename; fullpath=$datadir/$filename; sleep 5; root -b -q "xZgbkg.C(\\\"$fullpath\\\", \\\"$PUpath\\\", \\\"$IDSFpath\\\", \\\"$CSEVpath\\\")"; cp output_ggtree_mc.root $outfile; rm output_ggtree_mc.root; done; \n"""
