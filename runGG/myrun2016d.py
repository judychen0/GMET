...
with open(remotepath, "r") as pathfile:
    ...
    for line in path:
        alist = line.split("/")
        namelist = []
        namelist.append("summer16_"+alist[-6]+"_"+alist[-2])
        tmpname = "".join(namelist)
        dirname = tmpname.replace("job_", "")
        ...
        script += """for filename in $(cat path.txt); do outfile=output_$filename; fullpath=$datadir/$filename; sleep 5; root -b -q "xZgdata.C(\\\"$fullpath\\\")"; cp output_ggtree_data.root $outfile; rm output_ggtree_data.root; done; \n"""
        script += """echo "Processed all $count ggtrees!!!" \n"""
        script += "for i in {1..9}; do hadd output_ggtree_$i.root *data_$i*.root; done \n"
        script += "rm -r *data* \n"
        ...
        frun = open("run2016d.sh", "w")
        frun.write(script)
        frun.close()
        
        os.system( "chmod 744 run2016d.sh " )
        os.system( "./run2016d.sh")
