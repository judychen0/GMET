...
void xSkim_mc(char* pathes, char* PUpathes, char* IDpathes, char* PSVpathes, Int_t dcount, Int_t year, Int_t sig){
  ...
  TH2F *h_PSVSF = (TH2F*)fopen->Get("Scaling_Factors_PixelSeed_R9_Inclusive")->Clone();//Legacy MC
  //TH1F *h_PSVSF = (TH1F*)fopen->Get("MediumID/SF_HasPix_MediumID")->Clone();// UL MC
  ...
  Int_t npfjet_corr;
  vector<Float_t> jetIso;
  vector<Float_t> jetdR_lead;
  vector<Float_t> jetdR_sub;
  vector<Float_t> jetPt_corr;
  vector<Float_t> jetEta_corr;
  vector<Float_t> jetPhi_corr;
  vector<Float_t> jetEn_corr;
  ...
  outtree_->Branch("npfjet_corr" ,&npfjet_corr ,"npfjet_corr/I");
  outtree_->Branch("jetIso" ,&jetIso);
  outtree_->Branch("jetdR_lead" ,&jetdR_lead);
  outtree_->Branch("jetdR_sub" ,&jetdR_sub);
  outtree_->Branch("jetPt_corr" ,&jetPt_corr);
  outtree_->Branch("jetEta_corr" ,&jetEta_corr);
  outtree_->Branch("jetPhi_corr" ,&jetPhi_corr);
  outtree_->Branch("jetEn_corr" , &jetEn_corr);
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	Int_t ptbin = 0;
	Int_t etabin = 0;
	if(phoCalibEt[ipho] >= 200) ptbin = 5;
	//else if(phoCalibEt[ipho] >= 250 && phoCalibEt[ipho] < 300) ptbin = 2;
	//else if(phoCalibEt[ipho] >= 300 && phoCalibEt[ipho] < 350) ptbin = 3;
	//else if(phoCalibEt[ipho] >= 350 && phoCalibEt[ipho] < 450) ptbin = 4;
	//else if(phoCalibEt[ipho] >= 450 && phoCalibEt[ipho] < 1000) ptbin = 5;
	...
	PSVSF = h_PSVSF->GetBinContent(1, 1);// Legacy MC
	//PSVSF = h_PSVSF->GetBinContent(1);// UL MC
      }
      Double_t tot_wei = 1;
    ...
    //jet splitting energy correction
    Float_t leadjetIso = 0;
    Float_t subjetIso = 0;
    TVector3 leadjet;
    TVector3 subjet;
    TVector3 smalljet;
    TVector3 leadjet_corr;
    TVector3 subjet_corr;
    npfjet_corr = npfjet;
    ...
    if(npfjet>=3){
      ...
      leadjet.SetPtEtaPhi(jetPtSmear[ilead], jetEta_[ilead], jetPhi_[ilead]);
      subjet.SetPtEtaPhi(jetPtSmear[isub], jetEta_[isub], jetPhi_[isub]);

      jetPt_corr.push_back(jetPtSmear[ilead]);
      jetPt_corr.push_back(jetPtSmear[isub]);
      jetEta_corr.push_back(jetEta_[ilead]);
      jetEta_corr.push_back(jetEta_[isub]);
      jetPhi_corr.push_back(jetPhi_[ilead]);
      jetPhi_corr.push_back(jetPhi_[isub]);
      jetEn_corr.push_back(jetEn_[ilead]);
      jetEn_corr.push_back(jetEn_[isub]);
      
      for(Int_t jj=2; jj<npfjet; jj++){
	Int_t ijet = nonPUjetid[jj];
	Int_t count = 0;
	
	//if(jj>=2){
	  Float_t leadjet_deltaR = deltaR(jetEta_[ilead], jetPhi_[ilead], jetEta_[ijet], jetPhi_[ijet]);
	  Float_t subjet_deltaR = deltaR(jetEta_[isub], jetPhi_[isub], jetEta_[ijet], jetPhi_[ijet]);
	  smalljet.SetPtEtaPhi(jetPtSmear[ijet], jetEta_[ijet], jetPhi_[ijet]);
	  jetdR_lead.push_back(leadjet_deltaR);
	  jetdR_sub.push_back(subjet_deltaR);
	  
	  if(leadjet_deltaR<0.8) leadjetIso += smalljet.Mag();
	  if(subjet_deltaR<0.8) subjetIso += smalljet.Mag();
	  
	  //if(leadjet_deltaR<0.2 && leadjet_deltaR<subjet_deltaR && leadcount==0){
	  //  leadjet += smalljet;
	  //  leadcount++;
	  //  count++;
	  //  npfjet_corr-=1;
	  //  jetPt_corr[0] = leadjet.Pt();
	  //  jetEta_corr[0] = leadjet.Eta();
	  //  jetPhi_corr[0] = Phi_mpi_pi(leadjet.Phi());
	  //  jetEn_corr[0] = leadjet.Mag();
	  //}
	  if(subjet_deltaR<0.8 && subcount==0){
	    subjet += smalljet;
	    subcount++;
	    count++;
	    npfjet_corr-=1;
	    jetPt_corr[1] = subjet.Pt();
	    jetEta_corr[1] = subjet.Eta();
	    jetPhi_corr[1] = Phi_mpi_pi(subjet.Phi());
	    jetEn_corr[1] = subjet.Mag();
	    
	  }

	  if(count == 0){
	    jetPt_corr.push_back(jetPtSmear[ijet]);
	    jetEta_corr.push_back(jetEta_[ijet]);
	    jetPhi_corr.push_back(jetPhi_[ijet]);
	    jetEn_corr.push_back(jetEn_[ijet]);
	  }
	  
	  //}
      }

      jetIso.push_back(leadjetIso);
      jetIso.push_back(subjetIso);
    ...
    if(npfjet_corr>= 2){
      ...
      //mindjetMETPhi
      if(fabs(deltaPhi(jetPhi_corr[ilead], corrMETPhi)) < fabs(deltaPhi(jetPhi_corr[isub], corrMETPhi))) minJMETdPhi = deltaPhi(jetPhi_corr[ilead], corrMETPhi);
      else if(fabs(deltaPhi(jetPhi_corr[ilead], corrMETPhi)) > fabs(deltaPhi(jetPhi_corr[isub], corrMETPhi))) minJMETdPhi = deltaPhi(jetPhi_corr[isub], corrMETPhi);

      //dijet dEta & dPhi & dR
      jetjetdEta = deltaEta(jetEta_corr[ilead], jetEta_corr[isub]);
      jetjetdPhi = deltaPhi(jetPhi_corr[ilead], jetPhi_corr[isub]);
      jetjetdR = deltaR(jetEta_corr[ilead], jetPhi_corr[ilead], jetEta_corr[isub], jetPhi_corr[isub]);

      //dijet Mass
      leadjetP4.SetPtEtaPhiE(jetPt_corr[ilead], jetEta_corr[ilead], jetPhi_corr[ilead], jetEn_corr[ilead]);
      subjetP4.SetPtEtaPhiE(jetPt_corr[isub], jetEta_corr[isub], jetPhi_corr[isub], jetEn_corr[isub]);   
      ...
      for(Int_t ipho=0; ipho<nPho; ipho++){
	if(ipho>0) continue;
	phojetdR.push_back(deltaR(phoEta[ipho], phoPhi[ipho], jetEta_corr[ilead], jetPhi_corr[ilead]));
	phojetdR.push_back(deltaR(phoEta[ipho], phoPhi[ipho], jetEta_corr[isub], jetPhi_corr[isub]));

	phojetdEta.push_back(deltaEta(phoEta[ipho], jetEta_corr[ilead]));
	phojetdEta.push_back(deltaEta(phoEta[ipho], jetEta_corr[isub]));

	phojetdPhi.push_back(deltaPhi(phoPhi[ipho], jetPhi_corr[ilead]));
	phojetdPhi.push_back(deltaPhi(phoPhi[ipho], jetPhi_corr[isub]));
      }
      
      //jetMET dPhi
      JMETdPhi.push_back(deltaPhi(jetPhi_corr[ilead], corrMETPhi));
      JMETdPhi.push_back(deltaPhi(jetPhi_corr[isub], corrMETPhi));
    }
    else{
      minJMETdPhi = -999;
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      if(ipho>0) continue;
      if(npfjet_corr<2) continue;
      Int_t ilead = 0;
      Int_t isub = 1;
      
      Float_t j1mj2_Eta = jetEta_corr[ilead] - jetEta_corr[isub];
      Float_t j1pj2_Eta = jetEta_corr[ilead] + jetEta_corr[isub];
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      ...
      //cout << " create phov2";
      leadjetv2.SetMagPhi(jetPt_corr[ilead], jetPhi_corr[ilead]);
      subjetv2.SetMagPhi(jetPt_corr[isub], jetPhi_corr[isub]);
	...
	if(jetPt_[nonPUjetid[0]] >30) {bit = SetBit(9, bit); nSMjet = npfjet_corr;}
	else if(jetPt_[nonPUjetid[0]] <30) {bit = SetBit(9, bit); nSMjet = 0;}
      }
      else if(npfjet_corr == 0){								bit = SetBit(9, bit); nSMjet = npfjet_corr;}
	...
	if(jetPt_corr[ilead] >50 && jetPt_corr[isub] >30) {bit = SetBit(9, bit); nVBSjet = npfjet_corr;}
	if((jetEta_corr[ilead] * jetEta_corr[isub]) < 0)					bit = SetBit(10, bit);
