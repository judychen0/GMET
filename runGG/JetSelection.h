...
//Legacy dataset
void jet_LIDsel(Int_t year, TreeReader &data, vector<int>& passed){
  ...
  Float_t jetNEF_cut[3][4] = {{0.9, 0.9, 0.01, 0.9}, {0.9, 0.9, 0.99, 0.9}, {0.9, 0.9, 0.99, 0.9}};
  ...
  if(year == 2016){ 
    ...
    for(Int_t ijet=0; ijet<nJet; ijet++){
      ...
      if(jetNCH[ijet] < jetNCH_cut[iyear][iEta] && jetNCH_cut[iyear][iEta] >= 0) continue;
      if(iEta == 2){
	if((jetNEF[ijet] > jetNEF_cut[iyear][iEta]) || jetNEF_cut[iyear][iEta] <0.02) continue;
      }
      else{
	if(jetNEF[ijet] > jetNEF_cut[iyear][iEta] && jetNEF_cut[iyear][iEta] >= 0) continue;
      }
