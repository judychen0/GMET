...
void xSkim_mc(char* pathes, char* PUpathes, char* IDpathes, char* CSEVpathes, Int_t dcount, Int_t year, Int_t sig){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    */
    ...
    for(Int_t ipho=0; ipho<nPho_; ipho++){
      if(ipho>0) continue;
      if(isData_ == 0){
	if(phoSCEta_[ipho] > -1.479 && phoSCEta_[ipho] <- 0.8) phoSF = h_phoIDSF->GetBinContent(4, 5);
	else if(phoSCEta_[ipho] >= -0.8 && phoSCEta_[ipho] < 0.0) phoSF = h_phoIDSF->GetBinContent(5, 5);
	else if(phoSCEta_[ipho] >= 0.0 && phoSCEta_[ipho] < 0.8) phoSF = h_phoIDSF->GetBinContent(6, 5);
	else if(phoSCEta_[ipho] >= 0.8 && phoSCEta_[ipho] < 1.479) phoSF = h_phoIDSF->GetBinContent(7, 5);

	CSVSF = h_CSVSF->GetBinContent(1, 1);//r9 inclusive[2D 2016]
      }
      Double_t tot_wei = phoSF*CSVSF*puwei_*genWeightSign;
      EventWeight = tot_wei;
    }
	...
	Int_t k = mc_phoid[nn];
	Float_t dr = deltaR(phoEta_[ipho], phoPhi_[ipho], mcEta_[k], mcPhi_[k]);
	Float_t dpt = fabs((phoEt_[ipho] - mcPt_[k])/mcPt_[k]);
    ...
    for(Int_t ijet=0; ijet<nJet_; ijet++){
      ...
      if(jetPt_[ijet] < 0) jetPtSmear.push_back(0);
    ...
    for(Int_t i=0; i<nJet_; i++){
      ...
      if((jetPUFullID_[i] == 6) && (jetPtSmear[i] > 30) && fabs(jetEta_[i]) < 4.7 && (jetID_[i] == 6)){
	...
	JMETdPhi.push_back(deltaPhi(jetPhi_[ijet], pfMETPhi_));
      }
    }
    ...
    for(Int_t ipho=0; ipho<nPho_; ipho++){
      if(ipho > 0) continue;
      bit=0;
      if(dcount == 0 && phoIsMatch[ipho] == 1)                                   bit = SetBit(0, bit);
      if(dcount == 1 && phoIsMatch[ipho] == 0)                                   bit = SetBit(0, bit);
