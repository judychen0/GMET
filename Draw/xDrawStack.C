...
void xDrawStack(Int_t year){
  ...
  TH1F *H_djetMETPhi_cut[nfile][njet];
  TH1F *H_dr_phojet[nfile][njet];
  TH1F *H_dEta_phojet[nfile][njet];
  TH1F *H_dPhi_phojet[nfile][njet];
  ...
  TH1F *HA_djetMETPhi_cut[njet];
  TH1F *HA_dr_phojet[njet];
  TH1F *HA_dEta_phojet[njet];
  TH1F *HA_dPhi_phojet[njet];
  ...
  TH1F *HSig_djetMETPhi_cut[njet];
  TH1F *HSig_dr_phojet[njet];
  TH1F *HSig_dEta_phojet[njet];
  TH1F *HSig_dPhi_phojet[njet];
  ...
  for(Int_t i=0; i<nfile; i++){
	...
	H_jetpt_cut[i][jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetpt_cut_%i_jet%i", jj, kk));
	H_jetEta_cut[i][jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetEta_cut_%i_jet%i", jj, kk));
	H_jetPhi_cut[i][jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetPhi_cut_%i_jet%i", jj, kk));
	
	if(i==0){
	  HA_jetpt_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetpt_cut_%i_jet%i", jj, kk))->Clone();
	  HA_jetEta_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetEta_cut_%i_jet%i", jj, kk))->Clone();
	  HA_jetPhi_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetPhi_cut_%i_jet%i", jj, kk))->Clone();
	...
	if(i==nfile-2){
	  HSig_jetpt_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetpt_cut_%i_jet%i", jj, kk))->Clone();
	  HSig_jetEta_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetEta_cut_%i_jet%i", jj, kk))->Clone();
	  HSig_jetPhi_cut[jj][kk]		= (TH1F*)fopen->Get(Form("dijet/h_jetPhi_cut_%i_jet%i", jj, kk))->Clone();
    ...
    for(Int_t kk=0; kk<njet; kk++){
      H_djetMETPhi_cut[i][kk]	= (TH1F*)fopen->Get(Form("dijet/h_djetMETPhi_cut_jet%i", kk));
      H_dr_phojet[i][kk]		= (TH1F*)fopen->Get(Form("dijet/h_dr_phojet_jet%i", kk));
      H_dEta_phojet[i][kk]	= (TH1F*)fopen->Get(Form("dijet/h_dEta_phojet_jet%i", kk));
      H_dPhi_phojet[i][kk]	= (TH1F*)fopen->Get(Form("dijet/h_dPhi_phojet_jet%i", kk));

      if(i==0){

	HA_djetMETPhi_cut[kk]	= (TH1F*)fopen->Get(Form("dijet/h_djetMETPhi_cut_jet%i", kk))->Clone();
	HA_dr_phojet[kk]		= (TH1F*)fopen->Get(Form("dijet/h_dr_phojet_jet%i", kk))->Clone();
	HA_dEta_phojet[kk]	= (TH1F*)fopen->Get(Form("dijet/h_dEta_phojet_jet%i", kk))->Clone();
	HA_dPhi_phojet[kk]	= (TH1F*)fopen->Get(Form("dijet/h_dPhi_phojet_jet%i", kk))->Clone();

	HA_djetMETPhi_cut[kk]->Scale(scale[i]);	
	HA_dr_phojet[kk]->Scale(scale[i]);		
	HA_dEta_phojet[kk]->Scale(scale[i]);	
	HA_dPhi_phojet[kk]->Scale(scale[i]);

	HA_djetMETPhi_cut[kk]->SetFillStyle(3145);	
	HA_dr_phojet[kk]->SetFillStyle(3145);		
	HA_dEta_phojet[kk]->SetFillStyle(3145);	
	HA_dPhi_phojet[kk]->SetFillStyle(3145);

	HA_djetMETPhi_cut[kk]->SetFillColor(12);	
	HA_dr_phojet[kk]->SetFillColor(12);		
	HA_dEta_phojet[kk]->SetFillColor(12);	
	HA_dPhi_phojet[kk]->SetFillColor(12);

	HA_djetMETPhi_cut[kk]->SetMarkerSize(0.1);	
	HA_dr_phojet[kk]->SetMarkerSize(0.1);		
	HA_dEta_phojet[kk]->SetMarkerSize(0.1);	
	HA_dPhi_phojet[kk]->SetMarkerSize(0.1);

      }

      if(i==nfile-2){

	HSig_djetMETPhi_cut[kk]	= (TH1F*)fopen->Get(Form("dijet/h_djetMETPhi_cut_jet%i", kk))->Clone();
	HSig_dr_phojet[kk]		= (TH1F*)fopen->Get(Form("dijet/h_dr_phojet_jet%i", kk))->Clone();
	HSig_dEta_phojet[kk]	= (TH1F*)fopen->Get(Form("dijet/h_dEta_phojet_jet%i", kk))->Clone();
	HSig_dPhi_phojet[kk]	= (TH1F*)fopen->Get(Form("dijet/h_dPhi_phojet_jet%i", kk))->Clone();

	HSig_djetMETPhi_cut[kk]->Scale(scale[i]);	
	HSig_dr_phojet[kk]->Scale(scale[i]);		
	HSig_dEta_phojet[kk]->Scale(scale[i]);	
	HSig_dPhi_phojet[kk]->Scale(scale[i]);

	HSig_djetMETPhi_cut[kk]->SetLineColor(kRed);	
	HSig_dr_phojet[kk]->SetLineColor(kRed);		
	HSig_dEta_phojet[kk]->SetLineColor(kRed);	
	HSig_dPhi_phojet[kk]->SetLineColor(kRed);
	  
	HSig_djetMETPhi_cut[kk]->SetLineWidth(2);	
	HSig_dr_phojet[kk]->SetLineWidth(2);		
	HSig_dEta_phojet[kk]->SetLineWidth(2);	
	HSig_dPhi_phojet[kk]->SetLineWidth(2);

      }

      if(i == nfile-1){

	H_djetMETPhi_cut[i][kk]->SetLineWidth(2);	
	H_dr_phojet[i][kk]->SetLineWidth(2);		
	H_dEta_phojet[i][kk]->SetLineWidth(2);	
	H_dPhi_phojet[i][kk]->SetLineWidth(2);

	H_djetMETPhi_cut[i][kk]->SetMarkerStyle(8);	
	H_dr_phojet[i][kk]->SetMarkerStyle(8);		
	H_dEta_phojet[i][kk]->SetMarkerStyle(8);	
	H_dPhi_phojet[i][kk]->SetMarkerStyle(8);

	H_djetMETPhi_cut[i][kk]->SetMarkerSize(0.9);	
	H_dr_phojet[i][kk]->SetMarkerSize(0.9);		
	H_dEta_phojet[i][kk]->SetMarkerSize(0.9);	
	H_dPhi_phojet[i][kk]->SetMarkerSize(0.9);

      }

      H_djetMETPhi_cut[i][kk]->Scale(scale[i]);	
      H_dr_phojet[i][kk]->Scale(scale[i]);		
      H_dEta_phojet[i][kk]->Scale(scale[i]);	
      H_dPhi_phojet[i][kk]->Scale(scale[i]);

      if(i<nfile-1){
	H_djetMETPhi_cut[i][kk]->SetFillColor(cnum[i]);	
	H_dr_phojet[i][kk]->SetFillColor(cnum[i]);		
	H_dEta_phojet[i][kk]->SetFillColor(cnum[i]);	
	H_dPhi_phojet[i][kk]->SetFillColor(cnum[i]);
      }

      H_djetMETPhi_cut[i][kk]->SetLineColor(1);	
      H_dr_phojet[i][kk]->SetLineColor(1);		
      H_dEta_phojet[i][kk]->SetLineColor(1);	
      H_dPhi_phojet[i][kk]->SetLineColor(1);

      H_djetMETPhi_cut[i][kk]->SetLineWidth(1);	
      H_dr_phojet[i][kk]->SetLineWidth(1);		
      H_dEta_phojet[i][kk]->SetLineWidth(1);	
      H_dPhi_phojet[i][kk]->SetLineWidth(1);
  ...
  for(Int_t i=0; i<nfile-1; i++){
	...
	//2jets jj
	HA_djetMETPhi_cut[jj]->Add(H_djetMETPhi_cut[i][jj]);
	HA_dr_phojet[jj]->Add(H_dr_phojet[i][jj]);
	HA_dEta_phojet[jj]->Add(H_dEta_phojet[i][jj]);
	HA_dPhi_phojet[jj]->Add(H_dPhi_phojet[i][jj]);
      }
    }
  }
  ...
  THStack *HS_djetMETPhi_cut[njet];
  THStack *HS_dr_phojet[njet];
  THStack *HS_dEta_phojet[njet];
  THStack *HS_dPhi_phojet[njet];
  ...
  for(Int_t jj=0; jj<2; jj++){
    ...
    // 2jet jj
    HS_djetMETPhi_cut[jj] = new THStack(Form("HS_djetMETPhi_cut_jet%i", jj), "HS_djetMETPhi_cut");
    HS_dr_phojet[jj]	= new THStack(Form("HS_dr_phojet_jet%i", jj), "HS_dr_phojet");
    HS_dEta_phojet[jj]	= new THStack(Form("HS_dEta_phojet_jet%i", jj), "HS_dEta_phojet");
    HS_dPhi_phojet[jj]	= new THStack(Form("HS_dPhi_phojet_jet%i", jj), "HS_dPhi_phojet");
  ...
  for(Int_t i=0; i<nfile-1; i++){
	...
	HS_jetPhi_cut[jj][kk]->Add(H_jetPhi_cut[i][jj][kk]);	
      ...
      HS_dr_phojet[jj]->Add(H_dr_phojet[i][jj]);
      HS_dEta_phojet[jj]->Add(H_dEta_phojet[i][jj]);
      HS_dPhi_phojet[jj]->Add(H_dPhi_phojet[i][jj]);
  ...
  TString ltext[20] = {"#gamma+jet", "W(#rightarrowl#nu)+jets", "Z(#rightarrow#nu#nu)+jets", "Z(#rightarrowll)+jets", "Monoboson", "Diboson", "Triboson", "Wg#rightarrowl#nu#gamma", "Zg#rightarrowll#gamma", "ttJets", "Z#gamma#rightarrow#nu#nu#gamma(VBS)", "DATA"};//Znunug VBS
  ...
  HS_dijetMass_cut->GetXaxis()->SetLabelSize(0);
  HS_dijetMass_cut->GetXaxis()->SetRangeUser(200, 2000);
  HS_dijetMass_cut->SetMaximum(150);
  HS_dijetMass_cut->SetMinimum(1);
  lhs->Clear();
  lhs->SetNColumns(2);
  for(Int_t ifile=0; ifile<nfile; ifile++){
  ...
  for(Int_t jj=0; jj<nhisto; jj++){
    ...
    HS_phoEB_ptcut[jj]->GetXaxis()->SetRangeUser(200, 1000);
    ...
    hratio->GetXaxis()->SetRangeUser(200, 1000);
    hratio->Draw("EP");
    tg->Draw("LSAME");
    CMS_lumi(pad1, period, 0);
    c1->Update();
    ...
    //HS_ptoverMET_cut[jj]->GetYaxis()->ChangeLabel(1, -1, 0, -1, -1, -1, "");
    HS_ptoverMET_cut[jj]->GetXaxis()->SetLabelSize(0);
    HS_ptoverMET_cut[jj]->SetMaximum(150);
    HS_ptoverMET_cut[jj]->SetMinimum(1);
    if(jj==1){HS_ptoverMET_cut[jj]->SetMaximum(10000); HS_ptoverMET_cut[jj]->SetMinimum(0.1);}
    lhs->Clear();
    lhs->SetNColumns(2);
    ...
    HSig_djetMETPhi_cut[jj]->Draw("HISTSAME");
    HA_djetMETPhi_cut[jj]->Draw("E2 SAME");
    ...
    HS_djetMETPhi_cut[jj]->GetXaxis()->SetLabelSize(0);
    HS_djetMETPhi_cut[jj]->SetMaximum(100000);
    HS_djetMETPhi_cut[jj]->SetMinimum(1);
    if(jj==1){HS_djetMETPhi_cut[jj]->SetMaximum(100000); HS_djetMETPhi_cut[jj]->SetMinimum(0.1);}
    lhs->Clear();
    lhs->SetNColumns(2);
    for(Int_t ifile=0; ifile<nfile; ifile++){
      //if(ifile==10) continue;
      if(ifile == nfile-1) lhs->AddEntry(H_djetMETPhi_cut[ifile][jj], ltext[ifile], "PL");
      if(ifile == nfile-2) lhs->AddEntry(HSig_djetMETPhi_cut[jj], ltext[ifile], "L");
    ...
    c1->SaveAs(Form("%s/%s.pdf", saveto, title));
      ...
      pad1->cd();
      HS_dr_phojet[jj]->Draw("HIST");
      H_dr_phojet[nfile-1][jj]->Draw("SAME");
      HSig_dr_phojet[jj]->Draw("HISTSAME");
      HA_dr_phojet[jj]->Draw("E2 SAME");
      HS_dr_phojet[jj]->GetYaxis()->SetTitle("Events");
      HS_dr_phojet[jj]->GetXaxis()->SetTitle("");
      //HS_dr_phojet[jj]->GetYaxis()->ChangeLabel(1, -1, 0, -1, -1, -1, "");
      HS_dr_phojet[jj]->GetXaxis()->SetLabelSize(0);
      HS_dr_phojet[jj]->SetMaximum(10000);
      HS_dr_phojet[jj]->SetMinimum(1);
      if(jj==1){HS_dr_phojet[jj]->SetMaximum(10000); HS_dr_phojet[jj]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
	if(ifile == nfile-1) lhs->AddEntry(H_dr_phojet[ifile][jj], ltext[ifile], "PL");
	if(ifile == nfile-2) lhs->AddEntry(HSig_dr_phojet[jj], ltext[ifile], "L");
	if(ifile < nfile-1) lhs->AddEntry(H_dr_phojet[ifile][jj], ltext[ifile], "F");
      }
      lhs->Draw("SAME");
      title = HS_dr_phojet[jj]->GetName();
      pad2->cd();
      hratio = ratioplot(HS_dr_phojet[jj], H_dr_phojet[nfile-1][jj]);
      hratio->SetTitle("");
      ...
      pad1->cd();
      HS_dEta_phojet[jj]->Draw("HIST");
      H_dEta_phojet[nfile-1][jj]->Draw("SAME");
      HSig_dEta_phojet[jj]->Draw("HISTSAME");
      HA_dEta_phojet[jj]->Draw("E2 SAME");
      HS_dEta_phojet[jj]->GetYaxis()->SetTitle("Events");
      HS_dEta_phojet[jj]->GetXaxis()->SetTitle("");
      //HS_dEta_phojet[jj]->GetYaxis()->ChangeLabel(1, -1, 0, -1, -1, -1, "");
      HS_dEta_phojet[jj]->GetXaxis()->SetLabelSize(0);
      HS_dEta_phojet[jj]->SetMaximum(100000);
      HS_dEta_phojet[jj]->SetMinimum(1);
      if(jj==1){HS_dEta_phojet[jj]->SetMaximum(10000); HS_dEta_phojet[jj]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
	if(ifile == nfile-1) lhs->AddEntry(H_dEta_phojet[ifile][jj], ltext[ifile], "PL");
	if(ifile == nfile-2) lhs->AddEntry(HSig_dEta_phojet[jj], ltext[ifile], "L");
	if(ifile < nfile-1) lhs->AddEntry(H_dEta_phojet[ifile][jj], ltext[ifile], "F");
      }
      lhs->Draw("SAME");
      title = HS_dEta_phojet[jj]->GetName();
      pad2->cd();
      hratio = ratioplot(HS_dEta_phojet[jj], H_dEta_phojet[nfile-1][jj]);
      hratio->SetTitle("");
      ...
      pad1->cd();
      HS_dPhi_phojet[jj]->Draw("HIST");
      H_dPhi_phojet[nfile-1][jj]->Draw("SAME");
      HSig_dPhi_phojet[jj]->Draw("HISTSAME");
      HA_dPhi_phojet[jj]->Draw("E2 SAME");
      HS_dPhi_phojet[jj]->GetYaxis()->SetTitle("Events");
      HS_dPhi_phojet[jj]->GetXaxis()->SetTitle("");
      //HS_dPhi_phojet[jj]->GetYaxis()->ChangeLabel(1, -1, 0, -1, -1, -1, "");
      HS_dPhi_phojet[jj]->GetXaxis()->SetLabelSize(0);
      HS_dPhi_phojet[jj]->SetMaximum(100000);
      HS_dPhi_phojet[jj]->SetMinimum(1);
      if(jj==1){HS_dPhi_phojet[jj]->SetMaximum(10000); HS_dPhi_phojet[jj]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
	if(ifile == nfile-1) lhs->AddEntry(H_dPhi_phojet[ifile][jj], ltext[ifile], "PL");
	if(ifile == nfile-2) lhs->AddEntry(HSig_dPhi_phojet[jj], ltext[ifile], "L");
	if(ifile < nfile-1) lhs->AddEntry(H_dPhi_phojet[ifile][jj], ltext[ifile], "F");
      }
      lhs->Draw("SAME");
      title = HS_dPhi_phojet[jj]->GetName();
      pad2->cd();
      hratio = ratioplot(HS_dPhi_phojet[jj], H_dPhi_phojet[nfile-1][jj]);
      hratio->SetTitle("");
    ...
    for(Int_t kk=0; kk<njet; kk++){
      ...
      HS_jetpt_cut[jj][kk]->SetMaximum(100000);
      HS_jetpt_cut[jj][kk]->SetMinimum(1);
      if(jj==1){HS_jetpt_cut[jj][kk]->SetMaximum(10000); HS_jetpt_cut[jj][kk]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
	...
	if(ifile < nfile-1) lhs->AddEntry(H_jetpt_cut[ifile][jj][kk], ltext[ifile], "F");
      ...
      HS_jetEta_cut[jj][kk]->SetMaximum(100000);
      HS_jetEta_cut[jj][kk]->SetMinimum(1);
      if(jj==1){HS_jetEta_cut[jj][kk]->SetMaximum(10000); HS_jetEta_cut[jj][kk]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
	...
	if(ifile < nfile-1) lhs->AddEntry(H_jetEta_cut[ifile][jj][kk], ltext[ifile], "F");
      ...
      HS_jetPhi_cut[jj][kk]->SetMaximum(10000);
      HS_jetPhi_cut[jj][kk]->SetMinimum(1);
      if(jj==1){HS_jetPhi_cut[jj][kk]->SetMaximum(10000); HS_jetPhi_cut[jj][kk]->SetMinimum(0.1);}
      lhs->Clear();
      lhs->SetNColumns(2);
      for(Int_t ifile=0; ifile<nfile; ifile++){
	//if(ifile==10) continue;
