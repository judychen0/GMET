...
void xDrawCutflow(Int_t year){
 ...
 if(year==2016){
  ...
  string hexcolor[ncolor] = {"#9297db", "#ffea00", "#03a9f4", "#ff9e00", "#4caf50", "#ff3360", "#0066ff", "#F0F5E1","#DCF563", "#2F81A3" };
//string hexcolor[ncolor] = {"#7FB5F5", "#EA80FC", "#FF9100", "#FFF176", "#0066FF", "#69F0AE","#FF1744",  "#0066FF"};
  ...
  Float_t integral[10][nfile][nhisto];
  ...
  for(Int_t i=0; i<nfile; i++){
    ...
    for(Int_t jj=0; jj<nhisto; jj++){
      ...
      H_phoEB_pt_200[i][jj]->ClearUnderflowAndOverflow();
      H_phoEB_pt_M[i][jj]->ClearUnderflowAndOverflow();
      H_phoEB_pt_chworst[i][jj]->ClearUnderflowAndOverflow();
      //H_phoEB_pt_phoptoverMET[i][jj]->ClearUnderflowAndOverflow();
      ...
      H_phoEB_pt_HLT[i][jj]->ClearUnderflowAndOverflow();

      integral[0][i][jj] = H_phoEB_pt_200[i][jj]->Integral(1, -1);
      integral[1][i][jj] = H_phoEB_pt_M[i][jj]->Integral(1, -1);
      integral[2][i][jj] = H_phoEB_pt_chworst[i][jj]->Integral(1, -1);
      //H_phoEB_pt_phopt)overMET[i][jj]->Integral(1, -1);
      ...
      integral[7][i][jj] = H_phoEB_pt_HLT[i][jj]->Integral(1, -1);
      
      H_phoEB_pt_M[i][jj]->SetBinContent(22, 0);
      H_phoEB_pt_chworst[i][jj]->SetBinContent(22, 0);
  ...
  Float_t lumiscale[5] = {5.74, 0.00064, 0.0026, 0.02};
